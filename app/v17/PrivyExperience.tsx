...
import { usePrivyMotion, scrollToTarget, setLenisPaused } from './anim/usePrivyMotion';
...
function Dialog({ children, onClose, label, wide = false }: { children: ReactNode; onClose: () => void; label: string; wide?: boolean }) {
  ...
  useEffect(() => { const dialog = ref.current; dialog?.showModal(); setLenisPaused(true); return () => { dialog?.close(); setLenisPaused(false); }; }, []);
...
function InquiryForm({ selected = 'all' }: { selected?: string }) {
  ...
  return <form className={s.form} onSubmit={submit}>
    ...
    <button className={s.goldButton} data-magnetic="0.22" type="submit" disabled={status === 'loading'}><span>{status === 'loading' ? 'در حال ثبت درخواست…' : 'درخواست بازدید اختصاصی'}</span><ArrowUpLeft size={20} /></button>
...
export default function PrivyExperience() {
  ...
  const collectionRange = useRef(0);
  const { scrollYProgress: collectionProgress } = useScroll({ target: collectionRef, offset: ['start start', 'end end'] });
  const trackX = useTransform(collectionProgress, (value) => value * collectionRange.current);
  usePrivyMotion(experienceRef, !!reduced);
  ...
  useEffect(() => {
    ...
    const measure = () => {
      ...
      if (!desktop) {
        ...
        collectionRange.current = 0;
        return;
      }
      const range = Math.max(0, track.scrollWidth - window.innerWidth);
      collectionRange.current = range;
      section.style.height = `${window.innerHeight + range}px`;
    };
    measure();
  ...
  function goTo(id: string) { setMenu(false); scrollToTarget(id, !!reduced); }

  return <MotionConfig reducedMotion="user"><div className={s.experience} ref={experienceRef}>
    ...
    <header data-header className={`${s.header} ${scrolled ? s.headerScrolled : ''}`}>
      ...
      <div className={s.headerActions}><span className={s.language}>FA <span>⌄</span></span><span className={s.headerDivider} /><button className={s.headerBooking} data-magnetic="0.2" onClick={() => setBooking('all')}>رزرو بازدید <ArrowUpLeft size={16} /></button></div>
    </header>
    <main>
      <section ref={heroRef} className={s.hero} id="top">
        <motion.div className={s.heroVisual} style={{ y: reduced ? 0 : heroY }}><img className={s.heroPoster} src="/privy/images/hero.jpg" alt="روبان‌های فلزی درخشان در فضایی تاریک" fetchPriority="high" /><video ref={videoRef} className={s.heroVideo} src="/privy/intro.mp4" poster="/privy/images/hero.jpg" loop muted playsInline preload="metadata" aria-hidden="true" /><div className={s.heroTint} /></motion.div>
        <motion.div className={s.heroContent} style={{ opacity: reduced ? 1 : heroOpacity }}>
          <motion.p className={s.heroEyebrow} dir="ltr" initial={{ opacity: 0 }} animate={ready ? { opacity: 1 } : undefined} transition={{ delay: .25, duration: 1.2 }}>A COLLECTION BEYOND COMPARE</motion.p>
          <h1><motion.span className={s.heroFirstLine} initial={{ opacity: 0, y: 35 }} animate={ready ? { opacity: 1, y: 0 } : undefined} transition={{ delay: .3, duration: 1.4, ease }}>هنرِ</motion.span><motion.span className={s.heroSecondLine} initial={{ opacity: 0, y: 45 }} animate={ready ? { opacity: 1, y: 0 } : undefined} transition={{ delay: .5, duration: 1.5, ease }}>بی‌همتا زیستن</motion.span></h1>
          <motion.p className={s.heroSubtitle} initial={{ opacity: 0 }} animate={ready ? { opacity: 1 } : undefined} transition={{ delay: 1, duration: 1.3 }}>خانه‌هایی برای آنان که زندگی را متفاوت می‌بینند.</motion.p>
          <motion.span className={s.heroSignature} dir="ltr" initial={{ opacity: 0 }} animate={ready ? { opacity: 1 } : undefined} transition={{ delay: 1.1, duration: 1.2 }}>The art of the sublime</motion.span>
        </motion.div>
        <div className={s.heroSideLabel} dir="ltr">EXCLUSIVELY YOURS — PRIVY COLLECTION</div>
        <div className={s.heroBottom}>
          <div className={s.filmActions}><button className={s.filmButton} data-magnetic="0.16" onClick={() => setFilm(true)}><span className={s.circle}><Play size={15} fill="currentColor" /></span><span>تماشای روایت پریوی<small dir="ltr">THE PRIVY EXPERIENCE</small></span></button><button className={s.pauseButton} onClick={() => setPaused(!paused)} aria-label={paused ? 'پخش انیمیشن پس‌زمینه' : 'توقف انیمیشن پس‌زمینه'} aria-pressed={paused}>{paused ? <Play size={13} /> : <span className={s.equalizer}><i /><i /><i /><i /></span>}</button></div>
          ...
          <button className={s.mapTeaser} data-magnetic="0.12" onClick={() => goTo('locations')}><img src="/privy/images/villa.jpg" alt="" /><span className={s.mapTeaserShade} /><span className={s.mapTeaserText}><small>نشانی یک زندگی متفاوت</small><strong>کشف اقامتگاه‌ها</strong></span><span className={s.mapTeaserPlus}><Plus size={19} /></span></button>
        </div>
      ...
      <CinemaSection />
      <section className={s.collection} id="collection" ref={collectionRef}><div className={s.collectionViewport}><motion.div className={s.collectionTrack} ref={trackRef} style={{ x: reduced ? 0 : trackX }}>
        <div className={s.collectionHeading}><div className={s.sectionHeading}><Reveal><Eyebrow>مجموعهٔ منتخب</Eyebrow><h2 data-reveal="title">سه جهان،<br /><em>یک انتخاب بی‌همتا.</em></h2></Reveal><Reveal className={s.collectionIntro}><p>در ارتفاع، در آغوش طبیعت، در امتداد دریا.<br />جهان خود را پیدا کنید.</p><span className={s.english}>THREE WORLDS. ONE EXTRAORDINARY COLLECTION.</span></Reveal></div></div>
        <div className={s.collectionSlides}>{residences.map((r, index) => <Reveal key={r.id} delay={index * .12} className={s.collectionSlide}><button className={s.residenceCard} onClick={() => setSelected(r)}><span className={s.residenceImage}><img src={r.image} alt={r.title} loading="lazy" /><span className={s.cardShade} /><span className={s.cardNumber}>۰{digits(index + 1)}</span><span className={s.cardCategory}>{r.category}</span><span className={s.cardExplore}>کشف اقامتگاه <ArrowUpLeft size={20} /></span></span><span className={s.cardInfo}><span><small><MapPin size={12} />{r.location}</small><strong>{r.title}</strong></span><span className={s.cardArrow}><ArrowUpLeft size={24} strokeWidth={1} /></span></span><span className={s.cardEnglish} dir="ltr">{r.english}</span></button></Reveal>)}</div>
        <div className={s.collectionNoteWrap}><p className={s.collectionNote} data-reveal="fade-in"><span />خانه‌هایی به تعداد انگشتان دست؛ تجربه‌هایی به وسعت یک زندگی.<span /></p></div>
      </motion.div></div></section>
      <section data-theme-section="light" className={s.tenets} id="tenets"><div className={s.tenetPicture}><img data-parallax="backgroundMove" src="/privy/images/interior.webp" alt="طراحی داخلی با متریال طبیعی، نور و ظرافت" loading="lazy" /><div className={s.tenetImageShade} /><span className={s.tenetImageCaption} dir="ltr" data-parallax="title">Excellence is<br /><em>in the details.</em></span><span className={s.tenetImageFoot}>زیبایی، در جزئیات زندگی می‌کند.</span></div><div className={s.tenetContent}><Reveal><Eyebrow>اصول پریوی</Eyebrow><h2 data-reveal="title">معیار ما،<br /><em>فراتر از معمول.</em></h2><p className={s.muted}>چه چیزی یک خانه را به مجموعهٔ پریوی می‌رساند؟<br />چهار اصل، بدون هیچ مصالحه‌ای.</p></Reveal><div className={s.accordion}>{tenets.map((t, i) => <div className={`${s.tenetItem} ${tenet === i ? s.tenetActive : ''}`} key={t.title}><button onClick={() => setTenet(tenet === i ? -1 : i)} aria-expanded={tenet === i} aria-controls={`privy-tenet-${i}`}><span className={s.tenetNumber}>۰{digits(i + 1)}</span><span>{t.title}</span><Plus size={18} /></button><AnimatePresence initial={false}>{tenet === i && <motion.div id={`privy-tenet-${i}`} initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: .4, ease }}><div className={s.tenetDescription}><p>{t.text}</p><span dir="ltr">{t.english}</span></div></motion.div>}</AnimatePresence></div>)}</div></div></section>
      ...
      <section data-theme-section="light" className={s.contact} id="contact"><div className={s.contactText}><Reveal><Eyebrow>دعوتی شخصی برای شما</Eyebrow><h2 data-reveal="title">فصل تازهٔ زندگی،<br />با یک <em>آشنایی.</em></h2><p>برای شناخت نزدیک‌تر مجموعه و هماهنگی یک بازدید<br />اختصاصی، همراه شما هستیم.</p><div className={s.contactSignature}><Brand small /><span>انتخابی برای یک عمر.</span></div></Reveal></div><Reveal className={s.contactForm}><InquiryForm /></Reveal></section>
    </main>
    ...
    {booking && <Dialog label="درخواست بازدید اختصاصی" onClose={() => setBooking(null)}><div className={s.bookingPanel}><Eyebrow>یک دعوت اختصاصی</Eyebrow><h2 data-reveal="title">از نزدیک، <em>متفاوت است.</em></h2><p className={s.muted}>برای آغاز آشنایی، اطلاعات خود را با ما به اشتراک بگذارید.</p><InquiryForm selected={booking} /></div></Dialog>}
    {film && <Dialog label="روایت تصویری پریوی" onClose={() => setFilm(false)} wide><div className={s.filmPanel}><div className={s.filmTitle}><Eyebrow>روایت پریوی</Eyebrow><h2 data-reveal="title">هنرِ بی‌همتا زیستن</h2><p dir="ltr">THE ART OF THE SUBLIME</p></div><video src="/privy/intro.mp4" poster="/privy/images/hero.jpg" autoPlay={!reduced} controls playsInline loop aria-label="فیلم کوتاه روبان‌های مجموعهٔ پریوی" /><div className={s.filmFooter}><span>از جزئیات کوچک، تا احساسی بی‌انتها.</span><button className={s.textLink} onClick={() => { setFilm(false); goTo('collection'); }}>کشف مجموعه <ArrowLeft size={18} /></button></div></div></Dialog>}
  </div></MotionConfig>;
}
