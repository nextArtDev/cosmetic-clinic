...
import { ArrowDown, ArrowUpLeft, ArrowLeft, Plus, Minus, X, ShoppingBag, Menu, Check, Flower2 } from 'lucide-react';
...
export default function FlowersExperience({ products }: { products: Product[] }) {
 ...
 function scrollTo(id: string) { setPanel(null); document.getElementById(id)?.scrollIntoView({ behavior: reduced ? 'instant' : 'smooth' }); }
  ...
  <div inert={panel ? true : undefined}>
   <section className={s.hero} ref={hero} id="flowers-top">
    ...
    <header className={s.header}>
     ...
     <nav className={s.desktopNav} aria-label="ناوبری اصلی"><button onClick={() => scrollTo('flowers-story')}>نگاه ما <span>↗</span></button><button onClick={() => scrollTo('flowers-catalog')}>مجموعه گل‌ها <span>↗</span></button><button onClick={() => scrollTo('flowers-team')}>آدم‌های سیم <span>↗</span></button><button onClick={() => { setContactSent(false); setPanel('contact'); }}>گفت‌وگو با ما <span>↗</span></button></nav>
     <button className={s.catalogPreview} onClick={() => scrollTo('flowers-catalog')} aria-label="دیدن مجموعه گل‌ها"><video muted loop playsInline autoPlay={!reduced} poster="/flowers/quiet.webp" src="/flowers/blossom.mp4" /><span>مجموعه گل‌ها <ArrowUpLeft size={23} /></span><small>THE FLOWER EDIT — ۰۱</small></button>
     <button className={`${s.iconButton} ${s.mobileMenu}`} onClick={() => setPanel('menu')} aria-label="باز کردن منو"><Menu /></button>
    </header>
    <div className={s.heroSide}><span className={s.tinyLabel}>برای هر حال، یک گل</span>{categories.map((c, i) => <button key={c.id} onClick={() => { setCategory(c.id); scrollTo('flowers-catalog'); }}><span>({['خ','ک','ع','ل'][i]})</span><span>{c.label}</span><ArrowUpLeft size={14} /></button>)}</div>
    ...
    <div className={s.heroBottom}>
     ...
     <motion.h1 initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: .25, duration: 1, ease }}>کمی زیبایی، برای هر روز.<br />برای شما که <em>ظرافت را زندگی می‌کنید.</em></motion.h1>
   ...
   <section className={s.storySection} id="flowers-story">
    ...
    <div className={s.storyGrid}>
     <div className={s.storyIntro}><Reveal><h2>زیبایی،<br /><span>در ساده‌ترین شکل.</span></h2><p>ما در سیم، گل‌ها را برای یک مناسبت نمی‌چینیم.<br />برای یک حس می‌چینیم. حسی که به خانه می‌آید<br />و کمی بیشتر از یک لحظه، می‌ماند.</p></Reveal><div className={s.storyImage}><AnimatePresence mode="wait"><motion.img key={story} src={stories[story].image} alt={stories[story].title} initial={{ opacity: 0, scale: 1.05 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} transition={{ duration: .5 }} /></AnimatePresence><span>( S / {persian(story + 1)} )</span></div><div className={s.imageCaption}><span>طبیعت، بدون هیچ کلمه اضافه.</span><Flower2 size={20} strokeWidth={1} /></div></div>
