      ...
      <div className="nc:mx-auto   nc:max-w-7xl   nc:px-5   nc:sm:px-8">
        <div className="nc:grid   nc:gap-12   nc:lg:grid-cols-12">
          <div className="nc:lg:col-span-5">
            <SectionHeading
              eyebrow="پس از عمل"
              title={"هر آنچه باید بدانید\nپس از *کاشت مو*"}
              description="پرسش‌های رایج روزهای اول پس از عمل. بقیه را می‌توانید در خط واتساپ اختصاصی از ما بپرسید."
            />
            <Reveal delay={3} className="nc:mt-8   nc:flex   nc:flex-wrap   nc:gap-3">
              <Button href="/v5/diagnostic">پرسش از پزشک</Button>
                ...
                return (
                  ...
                  >
                    <button
                      ...
                      aria-controls={`faq-${i}`}
                      className="nc:group   nc:flex   nc:w-full   nc:items-start   nc:justify-between   nc:gap-6   nc:py-6   nc:text-start"
                    >
                      <span className="nc:flex   nc:items-start   nc:gap-4">
                        <span className="nc:mt-1.5   nc:text-[11px]   nc:font-bold     nc:text-sage">
                          {String(i + 1).padStart(2, "0")}
                        </span>
                        <span
                          className={`nc:text-[1.05rem]   nc:font-semibold   nc:leading-snug     nc:transition-colors   nc:sm:text-[1.15rem]  ${
                            isOpen ? "nc:text-ink" : "nc:text-ink/80 nc:group-hover:text-ink"
                          }`}
                        >
                          {item.q}
                        </span>
                    ...
                    <AnimatePresence initial={false}>
                      {isOpen && (
                        ...
                        >
                          <p className="nc:pb-7   nc:ps-9   nc:pe-4   nc:text-[15px]   nc:leading-relaxed   nc:text-graphite   nc:sm:ps-10">
                            {item.a}
                          </p>
                        </motion.div>
                      )}
