    ...
    <>
      ...
      >
        ...
        >
          ...
          <nav className="nc:hidden   nc:items-center   nc:gap-8   nc:lg:flex" aria-label="ناوبری اصلی">
            {nav.primary.map((item) => (
              <Link
                key={item.label}
                href={item.href}
                className="link-underline   nc:text-[13.5px]   nc:font-semibold     nc:text-ink/80   nc:transition-colors   nc:hover:text-ink"
              >
                {item.label}
              </Link>
            ))}
            ...
            >
              <span className="nc:relative   nc:block   nc:h-3.5   nc:w-6">
                <motion.span
                  ...
                  className="nc:absolute   nc:start-0   nc:top-0   nc:block   nc:h-[2px]   nc:w-full   nc:origin-center   nc:rounded-full   nc:bg-ink"
                />
                <motion.span
                  animate={open ? { opacity: 0, scaleX: 0 } : { opacity: 1, scaleX: 1 }}
                  transition={{ duration: 0.25 }}
                  className="nc:absolute   nc:start-0   nc:top-1/2   nc:block   nc:h-[2px]   nc:w-full   nc:-translate-y-1/2   nc:rounded-full   nc:bg-ink"
                />
                <motion.span
                  animate={open ? { rotate: -45, y: -7 } : { rotate: 0, y: 0 }}
                  transition={{ duration: 0.4, ease: [0.76, 0, 0.24, 1] }}
                  className="nc:absolute   nc:bottom-0   nc:start-0   nc:block   nc:h-[2px]   nc:w-full   nc:origin-center   nc:rounded-full   nc:bg-ink"
                />
              </span>
              <span className="nc:pointer-events-none   nc:absolute   nc:-bottom-6   nc:hidden   nc:text-[10px]   nc:font-semibold   nc:normal-case     nc:text-ink/50   nc:opacity-0   nc:transition-opacity   nc:group-hover:opacity-100   nc:lg:block">
                {open ? "بستن" : "منو"}
              </span>
            </button>
          </div>
