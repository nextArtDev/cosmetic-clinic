...
import { StaggerGroup, StaggerItem, scaleIn } from '../ui/Reveal'
    ...
    >
      <div className="nc:mx-auto nc:max-w-7xl nc:px-5 nc:sm:px-8">
        <div className="nc:flex nc:flex-col nc:gap-8 nc:lg:flex-row nc:lg:items-end nc:lg:justify-between">
          <SectionHeading
            eyebrow="Nos greffes de cheveux"
            title={"Trois profils,\ntrois *stratégies* d'implantation"}
            description="Une chute androgénétique masculine, une raréfaction diffuse féminine ou des follicules courbes n'appellent pas le même geste. Choisissez votre profil pour comprendre notre approche."
          />
        </div>
            ...
            <StaggerItem key={p.index} variants={scaleIn}>
              ...
              >
                ...
                <div className="nc:absolute nc:inset-0 nc:overflow-hidden">
                  <Image
                    src={p.image}
                    alt={`Greffe de cheveux ${p.title.toLowerCase()}`}
                    fill
                ...
                <div className="nc:absolute nc:inset-x-0 nc:bottom-0 nc:p-6 nc:sm:p-7">
                  ...
                  <div className="nc:mt-2 nc:flex nc:items-end nc:justify-between nc:gap-4">
                    <h3 className="letterpress-light nc:text-[clamp(1.6rem,2.6vw,2.1rem)] nc:font-semibold nc:leading-none nc:tracking-[-0.03em]">
                      {p.title}
                    </h3>
                    <span className="nc:relative nc:grid nc:size-11 nc:shrink-0 nc:place-items-center nc:overflow-hidden nc:rounded-full nc:border nc:border-white/40 nc:bg-white/10 nc:backdrop-blur nc:transition-all nc:duration-500 nc:group-hover:border-white nc:group-hover:bg-white nc:group-hover:text-ink">
                      <ArrowUpRight className="nc:absolute nc:size-5 nc:transition-transform nc:duration-500 nc:ease-[cubic-bezier(.16,1,.3,1)] nc:group-hover:-translate-y-6 nc:group-hover:translate-x-6" />
                      <ArrowUpRight className="nc:absolute nc:size-5 nc:-translate-x-6 nc:translate-y-6 nc:transition-transform nc:duration-500 nc:ease-[cubic-bezier(.16,1,.3,1)] nc:group-hover:translate-x-0 nc:group-hover:translate-y-0" />
                    </span>
                  </div>
