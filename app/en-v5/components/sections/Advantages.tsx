      ...
      <div className="nc:mx-auto nc:max-w-7xl nc:px-5 nc:sm:px-8">
        <div className="nc:grid nc:gap-12 nc:lg:grid-cols-12 nc:lg:gap-10">
          <div className="nc:lg:col-span-5">
            <SectionHeading
              eyebrow="Technique"
              title={'Les atouts majeurs\nde la *FUE Saphir*'}
              description="Une évolution de la FUE classique : la lame en saphir remplace l'acier pour ouvrir des canaux receveurs plus fins, plus nets et plus rapprochés."
            />

            <Reveal delay={3} className="nc:mt-10">
              <div
                ref={imgRef}
                className="nc:relative nc:aspect-[5/4] nc:overflow-hidden nc:rounded-[24px] nc:shadow-soft"
              >
                <motion.div
                  style={{ y }}
                  className="nc:absolute nc:inset-[-10%]"
                >
                  <Image
                    src="/v5/images/hero-detail.webp"
                    alt="Lame en saphir et greffons alignés sur un plateau"
                    fill
                    sizes="(min-width: 1024px) 38vw, 100vw"
                    className="nc:object-cover"
                  />
                </motion.div>
                <div className="frosted nc:absolute nc:bottom-4 nc:left-4 nc:rounded-full nc:px-4 nc:py-2 nc:text-[12px] nc:font-semibold nc:ring-1 nc:ring-white/60">
                  Canaux receveurs de 0,8 à 1,2 mm
                </div>
              </div>
            </Reveal>
          ...
          <div className="nc:lg:col-span-7">
            <StaggerGroup
              className="nc:grid nc:gap-4 nc:sm:grid-cols-2"
              amount={0.15}
            >
              <div ref={gridRef} className="nc:contents">
                {advantages.map((a, i) => (
                  <StaggerItem
                    key={a.index}
                    className={i % 2 === 1 ? 'nc:sm:translate-y-10' : ''}
                  >
                    <article
                    ...
                    >
                      <div className="nc:flex nc:items-start nc:justify-between nc:gap-4">
                        <span
                          className={`orbit-badge ${i === 1 ? 'nc:text-white' : 'nc:text-ink'}`}
                        >
                          {a.index}
                        </span>
                        <span
                      ...
                      <h4 className="nc:mt-8 nc:text-[1.35rem] nc:font-semibold nc:leading-tight nc:tracking-[-0.02em]">
                        {a.title}
                      </h4>
                      <p
                        className={`nc:mt-3 nc:text-[14.5px] nc:leading-relaxed ${i === 1 ? 'nc:text-white/70' : 'nc:text-graphite'}`}
                      >
                        {a.text}
                      </p>
                      <span
