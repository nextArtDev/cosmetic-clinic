 ...
 *
 * Perf note that matters more than the library choice: three 1080x1920 videos
 * decoding at once behind a `backdrop-blur-xl` circle is the real cost. The
 * videos here stay paused until the team layer is on screen, and pause again
 * when it leaves.
 */
...
const HeroZoom = ({
  ...
  const trackY = useTransform(scrollYProgress, T.videoPan, [0, -travel])
  ...
  /* --- only decode video while the team layer is actually visible --- */
  const playing = useRef(false)
  useMotionValueEvent(scrollYProgress, 'change', (p) => {
    const shouldPlay = p > T.teamIn[0] && p < 0.99
    ...
    <ReactLenis root>
      ...
      >
        ...
        >
          ...
          >
            {/* masked video track: pans up like the old sky image */}
            <motion.div
              ref={trackRef}
              style={{ y: trackY }}
