...
import { usePrefersReducedMotion, useWebglSupported } from './hooks'
 ...
 * `copy`, and fonts. Degrades to a CSS clip-path comparator when WebGL is
 * unavailable, and freezes ambient shader motion for `prefers-reduced-motion`.
 *
 * @example
 * <CosmeticComparator
 *   items={[
  ...
  return (
    ...
    >
          ...
          <div className="mx-auto flex max-w-5xl gap-2 overflow-x-auto px-4 py-2.5 sm:justify-center">
            {items.map((item, navIndex) => (
              <a
                key={item.id}
                href={`#stage-${item.id}`}
                className="whitespace-nowrap rounded-full border border-[var(--cc-border)] bg-white/[0.03] px-3 py-1 text-[10px] text-[var(--cc-ink-muted)] transition-colors hover:border-[var(--cc-accent)]/50 hover:text-[var(--cc-accent-soft)]"
              >
        ...
        <div className="flex flex-col xl:flex-row xl:flex-wrap xl:justify-center">
          {items.map((item, index) => (
            <div
              ...
              style={{
                zIndex: index + 1,
                marginBottom: index < items.length - 1 ? '-40vh' : 0,
              }}
            >
              <ComparisonStage
                item={item}
      ...
      <footer className="relative flex flex-col items-center gap-4 px-6 py-24 text-center">
        <span className="h-1.5 w-1.5 rotate-45 bg-[var(--cc-accent)]" />
        <p className="max-w-md text-sm leading-7 text-[var(--cc-ink-muted)]">
          {resolvedCopy.footnote}
        </p>
      </footer> */}
    </main>
  )
