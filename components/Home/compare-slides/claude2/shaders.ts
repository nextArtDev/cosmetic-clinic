...
/**
 * All 21 fragment shaders read a normalized `uProgress` (0–1) each frame and
...
/** 1–4. Lower on mid-tier GPUs (see effect-mesh.tsx) — trims fbm() cost,
 *  the single biggest per-pixel expense in these shaders, without turning
 *  any effect off outright. Quality scales down; the shader stays real. */
...
/**
 * Gold-edge ports - the transition mechanics used by the home page three.js
 * slider (../ZCompare.tsx), re-expressed against this module COMMON helpers
 * (fbm / hash21 / coverUv / fetchBefore). The originals lit their seam with
 * fwidth(), which needs the derivatives extension on WebGL1; here the seam
 * glow is an explicit band around mask = 0.5, so it renders identically on
 * WebGL1 and WebGL2.
 */
const GOLD_EDGE = /* glsl */ `
vec3 applyGoldEdge(float mask, vec3 color){
  float edge = 1.0 - smoothstep(0.0, 0.10, abs(mask - 0.5));
  return mix(color, vec3(0.90, 0.80, 0.60), edge * 0.85);
}
`;

/** liquid: a soft noise-driven wipe, left to right, with a molten seam. */
const FRAG_GOLD_LIQUID = /* glsl */ `
void main(){
  vec2 uv = vUv;
  float p = clamp(uProgress, 0.0, 1.0);
  float noise = (fbm(uv * 3.0 + p * 2.0 + uSeed) - 0.5) * 0.30;
  float threshold = p + noise;
  float mask = smoothstep(threshold - 0.05, threshold + 0.05, uv.x);

  vec2 buv = coverUv(uv);
  vec3 afterC = texture2D(uAfter, buv).rgb;
  vec3 beforeC = fetchBefore(buv, afterC);
  vec3 col = mix(afterC, beforeC, mask);
  col = applyGoldEdge(mask, col);
  gl_FragColor = vec4(col, 1.0);
}
`;

/** wave: a sine-modulated wipe whose edge ripples vertically. */
const FRAG_GOLD_WAVE = /* glsl */ `
void main(){
  vec2 uv = vUv;
  float p = clamp(uProgress, 0.0, 1.0);
  float wave = sin(uv.y * 10.0 + p * 5.0) * 0.05;
  float threshold = p + wave;
  float mask = smoothstep(threshold - 0.02, threshold + 0.02, uv.x);
...
/** blinds: 12 horizontal bands that open at randomized offsets. */
const FRAG_GOLD_BLINDS = /* glsl */ `
void main(){
  vec2 uv = vUv;
  float p = clamp(uProgress, 0.0, 1.0);
  float band = floor(uv.y * 12.0);
  float rand = hash21(vec2(band, uSeed));
  float localProgress = clamp((p - rand * 0.5) * 2.0, 0.0, 1.0);
  float mask = smoothstep(localProgress - 0.05, localProgress + 0.05, uv.x);
...
/** diagonal: a clean 45-degree sweep. */
const FRAG_GOLD_DIAGONAL = /* glsl */ `
void main(){
  vec2 uv = vUv;
  float p = clamp(uProgress, 0.0, 1.0);
  float diag = (uv.x + uv.y) * 0.5;
  float mask = smoothstep(p - 0.05, p + 0.05, diag);
...
/** crystalline: a 10x10 cell grid snaps to the after-image in random order. */
const FRAG_GOLD_CRYSTALLINE = /* glsl */ `
void main(){
  vec2 uv = vUv;
  float p = clamp(uProgress, 0.0, 1.0);
  vec2 grid = floor(uv * 10.0);
  float rand = hash21(grid + uSeed);
  float mask = 1.0 - smoothstep(rand * 0.8, rand * 0.8 + 0.1, p);
...
export const FRAGMENT_SHADERS: Record<TransitionEffect, string> = {
  ...
  liquid: COMMON + GOLD_EDGE + FRAG_GOLD_LIQUID,
  wave: COMMON + GOLD_EDGE + FRAG_GOLD_WAVE,
  blinds: COMMON + GOLD_EDGE + FRAG_GOLD_BLINDS,
  diagonal: COMMON + GOLD_EDGE + FRAG_GOLD_DIAGONAL,
  crystalline: COMMON + GOLD_EDGE + FRAG_GOLD_CRYSTALLINE,
}
