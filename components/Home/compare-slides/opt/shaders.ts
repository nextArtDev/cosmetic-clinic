import { QUALITY_HIGH, qualityPrelude } from './quality'
import type { RenderTier } from './render-capability'
import type { TransitionEffect } from './types'

/**
 * All 16 fragment shaders read a normalized `uProgress` (0–1) each frame and a
 * shared `fetchBefore()` helper that either samples a real "before" texture or
 * synthesizes an aged plate from the "after" texture. Nothing here is
 * procedure-specific — labeling lives entirely in `ComparisonItem`.
 *
 * Cost knobs come from `quality.ts` as preprocessor defines rather than
 * uniforms, so the lite tier genuinely compiles less work instead of
 * branching around it at runtime.
 */
...
// Normalized to the original 4-octave range (0.9375 max), so a 2-octave build
// lands on the same smoothstep thresholds. At FBM_OCTAVES 4 the factor is
// exactly 1.0 and output is bit-identical to the untiered version.
float fbm(vec2 p){
  float v = 0.0; float a = 0.5;
  for (int i = 0; i < FBM_OCTAVES; i++){ v += a * vnoise(p); p *= 2.03; a *= 0.5; }
  return v * (0.9375 / (1.0 - pow(0.5, float(FBM_OCTAVES))));
}
...
void main(){
  ...
  float surface = h + 0.05 * sin(uv.x * 16.0 + uTime * 1.6);
#if AMBIENT
  surface += 0.03 * sin(uv.x * 37.0 - uTime * 2.3)
...
#if AMBIENT
  // 420 cycles per viewport height aliases hard below dpr 2, so the lite tier
  // drops the scanline entirely rather than shimmering.
...
void main(){
  ...
  float warp = 0.10 * sin(uv.y * 7.0 + uTime * 0.9);
#if AMBIENT
  warp += 0.05 * sin(uv.y * 17.0 - uTime * 1.4);
#endif
...
void main(){
  ...
  vec2 off = vec2(0.0);
#if AMBIENT
  off.x = (hash21(g + floor(uTime * 12.0)) - 0.5) * 0.02 * fresh;
#endif
...
/** Petal bloom: an organic flower-shaped reveal radiating from the focal
 *  point, with blush edge-glow and drifting pollen. Reads like a bloom
 *  opening rather than the plain circular wipe of `iris`. */
...
/** Thread lift: golden suture lines sweep across, cinching the after-image
 *  toward each thread as it passes, with a traveling glint. */
...
/** Metamorphosis: the frame splits down a central seam like a chrysalis
 *  opening, each half easing outward as it reveals, with an iridescent
 *  shimmer along the seam. The heaviest study — used as the probe subject. */
...
const BODIES: Record<TransitionEffect, string> = {
  tear: FRAG_TEAR,
  peel: FRAG_PEEL,
  dissolve: FRAG_DISSOLVE,
  ripple: FRAG_RIPPLE,
  laser: FRAG_LASER,
  cells: FRAG_CELLS,
  kintsugi: FRAG_KINTSUGI,
  frost: FRAG_FROST,
  silk: FRAG_SILK,
  iris: FRAG_IRIS,
  ...
  bloom: FRAG_BLOOM,
  thread: FRAG_THREAD,
  chrysalis: FRAG_CHRYSALIS,
}

const build = (prelude: string) =>
  Object.fromEntries(
    (Object.keys(BODIES) as TransitionEffect[]).map((key) => [
      key,
      prelude + COMMON + BODIES[key],
    ]),
  ) as Record<TransitionEffect, string>

const CACHE = new Map<string, Record<TransitionEffect, string>>()

/** Full-quality sources. Also what the capability probe compiles and times. */
export const FRAGMENT_SHADERS_HIGH: Record<TransitionEffect, string> = (() => {
  const built = build('')
  CACHE.set(QUALITY_HIGH, build(QUALITY_HIGH))
  return built
})()

/** Tier-resolved sources, memoized so a demotion recompiles once, not per stage. */
export function getFragmentShaders(
  tier: RenderTier,
): Record<TransitionEffect, string> {
  const prelude = qualityPrelude(tier)
  let cached = CACHE.get(prelude)
  if (!cached) {
    cached = build(prelude)
    CACHE.set(prelude, cached)
  }
  return cached
}
