...
/**
 * "Atelier" — deep vitrine-black with a warm, desaturated champagne accent:
 * the palette of a private surgical suite at night, not a landing page. `gem`
 * is a muted garnet reserved only for "before" markers, so before/after read
 * as two distinct materials rather than two shades of gold.
 */
...
export const DEFAULT_COPY: ComparatorCopy = {
  ...
  heading: 'What changes, rendered in real time',
  subheading:
  ...
  footnote:
    'Results vary by patient and procedure. Provide matched before/after pairs for production use.',
}

export const FA_COPY: ComparatorCopy = {
  eyebrow: 'مطالعات موردیِ تحول',
  heading: 'آنچه تغییر می‌کند، به‌صورت زنده',
  subheading:
    'هر قاب دقیقاً همگام با اسکرول شما حرکت می‌کند. برای دیدن حالت اولیه، لمس‌نگه‌دارید.',
  ...
  footnote:
    'نتایج بسته به بیمار و نوع عمل متفاوت است. برای استفاده واقعی، جفت تصاویر پیش/پس از را ارائه دهید.',
}
