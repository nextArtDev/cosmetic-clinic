...
function outlinePoints(
  ...
  const a = (v: number) => `${inset + v}px`
  const b = (v: number) => `calc(100% - ${inset + v}px)`
  const halfH = wH / 2 + sk
  const halfV = wV / 2 + sk

  pt(a(0), a(0))
  if (sides.includes('top')) {
    pt(`calc(50% - ${halfH}px)`, a(0))
    pt(`calc(50% - ${wH / 2}px)`, a(d))
    pt(`calc(50% + ${wH / 2}px)`, a(d))
    pt(`calc(50% + ${halfH}px)`, a(0))
  }
  pt(b(0), a(0))
  if (sides.includes('right')) {
    pt(b(0), `calc(50% - ${halfV}px)`)
    pt(b(d), `calc(50% - ${wV / 2}px)`)
    pt(b(d), `calc(50% + ${wV / 2}px)`)
    pt(b(0), `calc(50% + ${halfV}px)`)
  }
  pt(b(0), b(0))
  if (sides.includes('bottom')) {
    pt(`calc(50% + ${halfH}px)`, b(0))
    pt(`calc(50% + ${wH / 2}px)`, b(d))
    pt(`calc(50% - ${wH / 2}px)`, b(d))
    pt(`calc(50% - ${halfH}px)`, b(0))
  }
  pt(a(0), b(0))
  if (sides.includes('left')) {
    pt(a(0), `calc(50% + ${halfV}px)`)
    pt(a(d), `calc(50% + ${wV / 2}px)`)
    pt(a(d), `calc(50% - ${wV / 2}px)`)
    pt(a(0), `calc(50% - ${halfV}px)`)
  }
  return pts
...
function buildClipPath(
  ...
  return `polygon(${outlinePoints(0, notchSides, notchSize, notchWidth, notchWidthV, notchSkew).join(', ')})`
}

/** Ring clip (border only): outer outline + inner outline with evenodd fill. */
function buildRingClipPath(
  borderWidth: number,
  notchSides: NCNotchSide[],
  notchSize: number,
  notchWidth: number,
...
export const NotchCard: React.FC<NotchCardProps> = ({
  ...
  const bwNum =
    typeof borderWidth === 'number' ? borderWidth : parseFloat(borderWidth) || 2

  const clipPath = hollow
    ? buildRingClipPath(
        bwNum,
        validSides,
        notchSize,
      ...
      style={
        {
          ...
          backgroundColor: hollow
            ? 'transparent'
            : (cardColor ?? 'var(--background, #050505)'),
          color: textColor ?? undefined,
          clipPath: hollow ? undefined : clipPath,
          ...style,
